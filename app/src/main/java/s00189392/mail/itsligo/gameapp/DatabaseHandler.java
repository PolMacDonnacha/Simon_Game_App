...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    List<HiScore> getScoresByName(String name) {
        SQLiteDatabase db = this.getReadableDatabase();
        List<HiScore> scoreList = new ArrayList<HiScore>();
        ...
        Cursor cursor = db.query(TABLE_HI_SCORES, new String[] {
                        KEY_PLAYER_NAME, KEY_GAME_DATE,KEY_SCORE }, KEY_PLAYER_NAME + "=?",
                new String[] { name}, null, null, KEY_SCORE, null);
        if (cursor != null) {
            // looping through all rows and adding to list
            if (cursor.moveToFirst()) {
                do {
